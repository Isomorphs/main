...
public class Laser_Reflection_2: MonoBehaviour
{
	...
	void Start ()
	{
		...
		laserActionMask = reflectionMask | refractionMask | BlockingSurface;
	...
	void Update ()
	{
		...
		while (remainingRange > 0f && (reflectionCount < max_reflection_number)) {
			...
			if (Physics.Raycast (laserRay1, out hit, remainingRange, laserActionMask)) {
				...
				if (hit.collider.gameObject.layer == 9) {
					...
					if (incidentAngle > 90f)
						incidentAngle = 180f - incidentAngle;
