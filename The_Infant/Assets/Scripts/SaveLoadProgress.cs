...
public class SaveLoadProgress : MonoBehaviour {
	...
	public void Save () {
		...
		FileStream file = File.Create(Application.persistentDataPath + "/" + Application.loadedLevelName + "Progress.dat");
	...
	public void Load () {
		...
		if(File.Exists (Application.persistentDataPath + "/" + Application.loadedLevelName + "Progress.dat")) {
			BinaryFormatter bFormatter = new BinaryFormatter ();
			FileStream file = File.Open(Application.persistentDataPath + "/"+ Application.loadedLevelName + "Progress.dat", FileMode.Open);
			Transform[] itemParent = GameObject.Find("Items").GetComponentsInChildren<Transform>();
			Transform[] triggerableParent = GameObject.Find ("Triggerables").GetComponentsInChildren<Transform>();
